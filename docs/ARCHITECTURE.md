...
User-level interface preferences use a separate settings repository. The current browser-backed adapter may use local storage, while React consumes only the repository contract. Project document identity and folder operations remain native/project-local concerns.
...
Equation lines and visual captions are classified before translation. PDF image
operators and complex vector-path pages provide an additional visual-content
signal. The translated reader must degrade conservatively: render validated
...
`LocalTranslationRuntime` owns loopback runtime inspection, the reproducible smoke test, and structured segment translation. The Tauri adapter owns HTTP access to Ollama; React does not call the Ollama API directly. PDF.js extracts text and renders deficient pages for bundled Tesseract.js OCR. Persistence remains behind native project commands.
