...
## Closed: Jano-generated translation for 0.1

The first MVP definition described two selectable PDFs and a dual-PDF viewer.
The implemented 0.1 flow supersedes that assumption:

```text
left:  original PDF
right: clean Markdown generated by Jano
```

The user explicitly starts the local extraction, conditional English OCR,
translation, and review pipeline. The resulting segment IDs are the 0.1
alignment contract. External translated PDFs, Markdown, and text are deferred;
0.1 does not import, normalize, regenerate, or synchronize them. Existing files
are preserved and may be discovered, but their presence is not a supported
bilingual reader flow. See `adr/0013-jano-generated-translations-for-0.1.md`.

## Closed: hybrid `.jano` storage target

The current versioned JSON layout remains the `v0.1.0-rc.0` implementation so
the release candidate does not combine reader validation with a storage
migration. Before editable alignment, annotations, trash history, or larger
indexes, Jano will adopt a hybrid layout:

- JSON for the small bootstrap/configuration file and explicit portable exports;
- SQLite for canonical structured mutable state and relationships;
- ordinary files for original PDFs, readable generated Markdown/TXT, and bulky
  or inspectable derived artifacts.

There is one owner for each datum; JSON exports do not become a second writable
source of truth. SQLite remains behind the native repository boundary and raw
tables are not a frontend or plugin contract. See
`adr/0014-hybrid-project-storage.md` and `STORAGE-EVALUATION.md`.

## Future: external translation alignment algorithm
...
A separate post-0.1 design spike should choose the first production alignment algorithm.
...
- external translation import and alignment
